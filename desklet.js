...
MyDesklet.prototype = {
	...
	_init: function(metadata, desklet_id) {
		...
		for (let i = 0; i < 7; i++) {
			...
			this.labelWeekdays[i].set_text(Calendar.WEEKDAY_NAMES[i].substring(0, 2));
	...
	updateCalendar: function() {
        ...
        else this.labelDay.style = "color: " + this.colourText + "; " + STYLE_TEXT_CENTER;
        ...
        let col = (new Date(this.date.getFullYear(), this.date.getMonth(), 1)).getDay();
        col = (col + 6) % 7;			
		for (let i = 0, row = 2, 
	...
	updateValues: function() {
		...
		//////// Today Panel ////////
        let indexx = now.getDay();
        if (indexx === 0 ) indexx = 6; else indexx--;
		this.labelDay.set_text(Calendar.WEEKDAY_NAMES[indexx].substring(0, this.showWeekday !== "full" ? 3 : 9));
